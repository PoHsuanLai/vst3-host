...
use crate::ffi::{
    IComponentHandler2Vtable, IComponentHandler3Vtable,
    IComponentHandlerBusActivationVtable, IComponentHandlerVtable, K_NOT_IMPLEMENTED,
    K_RESULT_OK, IID_ICOMPONENT_HANDLER, IID_ICOMPONENT_HANDLER2, IID_ICOMPONENT_HANDLER3,
    IID_ICOMPONENT_HANDLER_BUS_ACTIVATION,
};
...
// Safety: ComponentHandler only contains thread-safe types
...
impl ComponentHandler {
    ...
    ///
    /// Returns the handler and a receiver for parameter edit events.
...
unsafe extern "system" fn handler_query_interface(
    ...
    if *iid_ref == IID_ICOMPONENT_HANDLER2 {
        // Return pointer to vtable2 location
...
unsafe extern "system" fn handler_begin_edit(this: *mut c_void, param_id: u32) -> i32 {
    let handler = &*(this as *const ComponentHandler);
    let _ = handler.event_sender.send(ParameterEditEvent::BeginEdit(param_id));
    K_RESULT_OK
}
...
unsafe extern "system" fn handler_perform_edit(
    ...
    let handler = &*(this as *const ComponentHandler);
    let _ = handler.event_sender.send(ParameterEditEvent::PerformEdit {
        param_id,
        value,
    });
    K_RESULT_OK
...
unsafe extern "system" fn handler_end_edit(this: *mut c_void, param_id: u32) -> i32 {
    let handler = &*(this as *const ComponentHandler);
    let _ = handler.event_sender.send(ParameterEditEvent::EndEdit(param_id));
    K_RESULT_OK
}
...
unsafe extern "system" fn handler2_query_interface(
    ...
    // Get base handler from vtable2 pointer offset
    let vtable2_ptr = this as *const *const IComponentHandler2Vtable;
    let handler_ptr = (vtable2_ptr as *const u8).sub(std::mem::offset_of!(ComponentHandler, vtable2))
...
unsafe extern "system" fn handler3_query_interface(
    ...
    let handler_ptr = (vtable3_ptr as *const u8).sub(std::mem::offset_of!(ComponentHandler, vtable3))
        as *mut ComponentHandler;
    handler_query_interface(handler_ptr as *mut c_void, iid, obj)
}
...
unsafe extern "system" fn handler3_create_context_menu(
    ...
    // Context menu creation not implemented - return null
    std::ptr::null_mut()
}
...
unsafe extern "system" fn handler_bus_query_interface(
    ...
    let handler_ptr = (vtable_bus_ptr as *const u8)
        .sub(std::mem::offset_of!(ComponentHandler, vtable_bus)) as *mut ComponentHandler;
    handler_query_interface(handler_ptr as *mut c_void, iid, obj)
}
